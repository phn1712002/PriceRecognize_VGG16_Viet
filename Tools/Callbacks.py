...
def createCallbacks(PATH_TENSORBOARD, PATH_LOGS, config, train_dataset, dev_dataset, pipeline):
    ...
    checkpoint_callbacks = ModelCheckpoint(filepath=PATH_LOGS + NAME_TIME + 'weights-{epoch:03d}.h5', save_best_only=True, save_weights_only=True, **config['config_train']['checkpoint'])
